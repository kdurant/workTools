...
class dacWidget(QWidget):
    ...
    def __init__(self):
        ...
        self.chDigtalValue.addItems(list(map(str, range(0, 5000, 5))))
    ...
    def configFrame(self):
        ...
        elif self.chSelectComb.currentText() == 'DA7':
            data_front = '0000 0007'

        data_back = str(hex(int(self.chDigtalValue.currentText(), 10))).replace(
            '0x', '').zfill(8)

        data = data_front + data_back

        self.packet.config(head=head, cmd_num=cmd_num,
                           command=command, data_len=data_len, data=data)
